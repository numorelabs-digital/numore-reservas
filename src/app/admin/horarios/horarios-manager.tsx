  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="flex gap-1 mb-4 p-1 card w-fit">
        {[["rec", "Recurrentes"], ["ses", "Sesiones"]].map(([k, label]) => (
    ...
    <div className="space-y-5">
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-sm font-semibold">Tipos de clase</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {classTypes.length === 0 && <p className="text-xs text-[var(--muted)]">Creá un tipo de clase primero.</p>}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="card divide-y divide-[var(--border)]">
          {schedules.length === 0 && <p className="p-4 text-sm text-[var(--muted)]">Sin horarios.</p>}
          {schedules.map((s: any) => (
      ...
      {/* Modal horario */}
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Editar horario" : "Nuevo horario"}>
      ...
      {/* Modal tipo */}
      <Modal open={typeOpen} onClose={() => setTypeOpen(false)} title="Nuevo tipo de clase">
  ...
  return (
    <div className="space-y-3">
      <button onClick={generar} disabled={pending}
        ...
        Generar sesiones (21 días)
      </button>

      <div className="card divide-y divide-[var(--border)]">
        {sessions.length === 0 && <p className="p-4 text-sm text-[var(--muted)]">Sin sesiones. Generalas desde los horarios.</p>}
        {sessions.map((s: any) => (
          <div key={s.id} className={cn("flex items-center gap-3 p-3.5", s.status !== "open" && "opacity-60")}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[var(--muted)]">
                {format(parseISO(s.session_date), "EEE d MMM", { locale: es })} · {s.start_time.slice(0, 5)} · {s.taken}/{s.capacity}
              </p>
            </div>
            <button onClick={() => editCap(s)} className="text-xs text-[var(--muted)] hover:text-[var(--text)] underline">cupo</button>
