...
// Crea un pago PIX de Mercado Pago para el paquete elegido.
export async function createPayment(packageId: string) {
  ...
  if (!pkg) return { ok: false as const, error: "Paquete no disponible." };

  // 1) Fila de pago pendiente (RLS: el alumno puede insertar la suya)
  const { data: payment, error: insErr } = await supabase
    ...
    .select("id").single();
  if (insErr || !payment) return { ok: false as const, error: "No se pudo iniciar el pago." };
  ...
  try {
    ...
    const pix = await createPixPayment({
      ...
      payerName: profile?.full_name ?? "Alumno",
      reference: payment.id,
      notificationUrl: `${base}/api/payments/webhook`,
    });

    // 3) Guardar datos del QR (service-role: actualiza sin restricción de RLS)
...
// El alumno consulta si su pago ya fue acreditado (para actualizar la pantalla).
