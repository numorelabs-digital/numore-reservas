...
export function CalendarClient() {
  ...
  const { data, mutate, isLoading } = useCalendar(profile?.id);
  ...
  const daySessions = sessions.filter((s) => isSameDay(parseISO(s.session_date), selected));
  ...
  function handleCancel(s: Session) {
    ...
    startTransition(async () => {
      ...
      setBusyId(null);
      if (res.ok) { toast.success("Reserva cancelada"); mutate(); }
      else toast.error(res.error);
    });
  }
      ...
      <div className="space-y-3">
        ...
        {daySessions.map((s) => (
          <SessionRow key={s.id} s={s} busy={pending && busyId === s.id}
            rescheduleMode={!!rescheduling} isBeingMoved={rescheduling?.sessionId === s.id}
            onBook={() => handleBook(s)} onCancel={() => handleCancel(s)} onReschedule={() => handleStartReschedule(s)} />
        ))}
      </div>
    </div>
  );
...
function SessionRow({
  ...
  return (
    <div className={cn("card p-4 flex items-center justify-between animate-fade-up", isBeingMoved && "opacity-50")}>
      ...
      {rescheduleMode ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onCancel} disabled={busy}
            className="text-xs font-medium rounded-lg border border-[var(--border)] px-2.5 py-2 hover:bg-[var(--bg)] disabled:opacity-50 flex items-center gap-1">
            {busy ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} className="text-green-500" />} OK
          </button>
        </div>
      ) : s.is_full ? (
